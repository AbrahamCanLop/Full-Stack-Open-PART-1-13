      ...
      if (confirmUpdate) {
        personService
          ...
          .then((updatedPerson) => {
            ...
            setNewName('')
            setNewNumber('')
            setNotification({message: `Added ${updatedPerson.name}`, type: 'success'})
            setTimeout(() => {
              setNotification({ message: null, type: '' })
            }, 5000)
          })
          .catch(error => {
            setNotification({message: `The person '${personSearch.name}' was already updated from the server`, type: 'error'})
            setPersons(persons.filter(p => p.id !== personSearch.id))
            setTimeout(() => {
              setNotification({ message: null, type: '' })
            }, 5000)
          })
